...
use stats::Stats;
use util::{PrinterPath, Replacer, Sunk};
...
impl<W: WriteColor> Standard<W> {
    ...
    /// exist. The stats returned are cumulative over all searched performed
    /// using this printer.
...
/// path.
#[derive(Debug)]
pub struct StandardSink<'p, 's, M: 's + Matcher, W: 's> {
    matcher: M,
...
impl<'p, 's, M: Matcher, W: WriteColor> StandardSink<'p, 's, M, W> {
    ...
    fn record_matches(
        ...
        if !self.needs_match_granularity(searcher) {
            return Ok(());
        }
    ...
    fn needs_match_granularity(&self, searcher: &Searcher) -> bool {
        ...
        || (!multi_line && self.standard.config.line_per_match)
...
impl<'p, 's, M: Matcher, W: WriteColor> Sink for StandardSink<'p, 's, M, W> {
    ...
    fn matched(
        ...
        self.record_matches(searcher, mat.bytes())?;
        ...
        let imp = StandardImpl::from_match(searcher, self, mat);
        if !imp.sink()? || imp.should_quit() {
            return Ok(false);
        }
        Ok(true)
    }
    ...
    fn context(
        ...
        if searcher.invert_match() {
            self.record_matches(searcher, ctx.bytes())?;
            self.replace(ctx.bytes())?;
        }
        let imp = StandardImpl::from_context(searcher, self, ctx);
        if !imp.sink()? || imp.should_quit() {
            return Ok(false);
        }
        Ok(true)
...
impl<'a, M: Matcher, W: WriteColor> StandardImpl<'a, M, W> {
    ...
    fn sink(&self) -> io::Result<bool> {
        if self.sunk.matches().is_empty() {
            if self.searcher.multi_line() && !self.is_context() {
    ...
    fn sink_fast(&self) -> io::Result<bool> {
        debug_assert!(self.sunk.matches().is_empty());
        debug_assert!(!self.searcher.multi_line() || self.is_context());
    ...
    fn sink_fast_multi_line(&self) -> io::Result<bool> {
        debug_assert!(self.sunk.matches().is_empty());
    ...
    fn sink_slow(&self) -> io::Result<bool> {
        debug_assert!(!self.sunk.matches().is_empty());
        debug_assert!(!self.searcher.multi_line() || self.is_context());
    ...
    fn sink_slow_multi_line(&self) -> io::Result<bool> {
        debug_assert!(!self.sunk.matches().is_empty());
        debug_assert!(self.searcher.multi_line());
        ...
        if self.config().only_matching {
            ...
            return self.sink_slow_multi_line_per_match();
        }
        ...
        let mut stepper = LineStep::new(
            line_term,
            Match::new(0, bytes.len()),
        );
        while let Some(mut line) = stepper.next(bytes) {
            self.write_prelude(
    ...
    fn sink_slow_multi_line_only_matching(&self) -> io::Result<bool> {
        let line_term = self.searcher.line_terminator();
        let spec = self.config().colors.matched();
        let bytes = self.sunk.bytes();
        let matches = self.sunk.matches();
    ...
    fn sink_slow_multi_line_per_match(&self) -> io::Result<bool> {
        let line_term = self.searcher.line_terminator();
        let spec = self.config().colors.matched();
        let bytes = self.sunk.bytes();
        for &m in self.sunk.matches() {
    ...
    /// Return the total number of times `Sink::matched` has been called for
    /// the current search.
    fn match_count(&self) -> u64 {
        self.sink.match_count
    }

    /// Return the total number of remaining lines that need to be printed to
    /// satisfy the `after_context` configuration knob.
    ///
    /// This is never greater than the configured `after_context` amount.
    fn after_context_remaining(&self) -> u64 {
        self.sink.after_context_remaining
    }
...
";
    ...
    #[test]
    fn scratch() {
        // let matcher = RegexMatcher::new(r"(?s)Watson.+Watson").unwrap();
        // let matcher = RegexMatcher::new(r"Sherlock|Watson").unwrap();
